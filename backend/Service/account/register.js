...
async function Create(Request) {
    ...
    const user = await (new Query).Select('user_id').From(Request.role).Where('user_id=$1', Request.user.id).Execute(0);
    if (user) {
        const updateModel = {role: Request.role}
        const result = await (new Query).UpdateColumns('users', updateModel, 'id', Request.user.id);
    ...
    await (new Query).UpdateColumns('users', updateModel, 'id', Request.user.id);
